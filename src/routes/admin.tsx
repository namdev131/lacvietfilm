...
import { Crown, Film, Loader2, Plus, RefreshCw, Shield, ShieldCheck, Trash2, Users } from "lucide-react";
import { toast } from "sonner";
import { useAuth } from "@/hooks/useAuth";
import { adminRole, type AdminRole } from "@/lib/admin-role";

type AdminUser = { id: string; email: string; display_name: string | null; role: AdminRole; created_at: string; last_sign_in_at: string | null };
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 pb-32 pt-8">
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div><p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-widest text-primary">{myRole === "admin" ? <Crown className="h-4 w-4" /> : <ShieldCheck className="h-4 w-4" />} {myRole === "admin" ? "Lạc Việt Admin" : "Phó Admin"}</p><h1 className="text-3xl font-black">Dashboard Admin</h1></div>
...
function UserRow({ item, adminId, actorRole, busy, act }: { item: AdminUser; adminId: string; actorRole: AdminRole; busy: boolean; act: (body: Record<string, unknown>, message: string) => Promise<void> }) {
  const [email, setEmail] = useState(item.email);
  const [displayName, setDisplayName] = useState(item.display_name || "");
  const protectedAdmin = item.role === "admin" && actorRole !== "admin";
  return <tr className="border-b border-border/60"><td className="p-3"><input disabled={protectedAdmin} value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-transparent disabled:opacity-70" /></td><td><input disabled={protectedAdmin} value={displayName} onChange={(e) => setDisplayName(e.target.value)} className="w-full bg-transparent disabled:opacity-70" /></td><td><span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-bold ${item.role === "admin" ? "bg-primary/15 text-primary" : item.role === "deputy_admin" ? "bg-secondary text-secondary-foreground" : "bg-muted text-muted-foreground"}`}>{item.role === "admin" ? <Crown className="h-3.5 w-3.5" /> : item.role === "deputy_admin" ? <ShieldCheck className="h-3.5 w-3.5" /> : null}{item.role === "admin" ? "Lạc Việt Admin" : item.role === "deputy_admin" ? "Phó Admin" : "Thành viên"}</span></td><td>{new Date(item.created_at).toLocaleDateString("vi-VN")}</td><td>{item.last_sign_in_at ? new Date(item.last_sign_in_at).toLocaleString("vi-VN") : "-"}</td><td className="p-3 text-right">{actorRole === "admin" && item.role !== "admin" && <button disabled={busy} onClick={() => void act({ action: "setDeputy", id: item.id, enabled: item.role !== "deputy_admin" }, item.role === "deputy_admin" ? "Đã gỡ Phó Admin" : "Đã gắn Phó Admin")} className="mr-2 border border-border px-3 py-1.5">{item.role === "deputy_admin" ? "Gỡ Phó Admin" : "Gắn Phó Admin"}</button>}<button disabled={busy || protectedAdmin} onClick={() => void act({ action: "updateUser", id: item.id, email, displayName }, "Đã cập nhật")} className="mr-2 border border-border px-3 py-1.5 disabled:opacity-40">Lưu</button><button disabled={busy || item.id === adminId || protectedAdmin} onClick={() => confirm("Xóa người dùng này? Dữ liệu liên quan cũng sẽ bị xóa.") && void act({ action: "deleteUser", id: item.id }, "Đã xóa người dùng")} className="bg-destructive px-3 py-1.5 text-destructive-foreground disabled:opacity-40"><Trash2 className="h-4 w-4" /></button></td></tr>;
}
